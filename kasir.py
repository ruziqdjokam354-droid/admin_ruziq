...
if not st.session_state.login:
    st.title("🔐 Login Kasir By Ruziq")

    username = st.text_input("Username")
    password = st.text_input("Password", type="password")

    if st.button("Login"):
        user_df = read_table("user_kasir")
        cek = user_df[
...
# LOAD DATA
# ========================
master_df = read_table("master_barang")
...
try:
    ...
    st.info("Tabel transaksi belum ada atau kosong")
...
with tab1:
    ...
    master_df_fresh = read_table("master_barang")
    ...
    if simpan:
        ...
        master_df_submit = read_table("master_barang")
        ...
        if insert_transaksi(data_transaksi):
            # Update stok
            new_stok = stok - jumlah
            if update_stok(st.session_state.barang_dipilih, new_stok):
                ...
                st.error("❌ Transaksi berhasil tapi gagal update stok")
        else:
    ...
    master_df_monitoring = read_table("master_barang")
...
with tab2:
    ...
    df_transaksi = read_table("transaksi")
    ...
    if not df_transaksi.empty:
        ...
        with col2:
            if st.button("🗑️ Hapus Transaksi"):
                # Dapatkan ID transaksi yang akan dihapus
                transaksi_id = df_transaksi.iloc[selected_row - 1]["id"]
                if delete_transaksi(transaksi_id):
...
with tab3:
    ...
    master_inv = read_table("master_barang")
    ...
    if submit_tambah:
        ...
        if tambah_qty_val is None or tambah_qty_val <= 0:
            ...
            if len(idx) == 0:
                ...
                new_stok = master_inv.loc[idx[0], "stok"] + tambah_qty_val
                if update_stok(pilih_barang, new_stok):
                    st.success(f"✅ Stok untuk {pilih_barang} berhasil ditambahkan sebanyak {tambah_qty_val}")
                    st.rerun()
                else:
                    st.error("❌ Gagal update stok")
...
with tab4:
    ...
    if not df_dash.empty:
        ...
        master_for_modal = read_table("master_barang")
        modal_col = None
